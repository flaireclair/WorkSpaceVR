...
namespace ListView
{
    ...
    public abstract class ListViewControllerBase : MonoBehaviour
    {
        ...
        protected virtual void ComputeConditions()
        {
            ...
            m_DownSide = transform.position + Vector3.up * range * 0.18f;
        ...
        {
            scrollOffset = index * itemSize.y;
        }

        protected virtual void Positioning(Transform t, int offset)
        {
            t.position = m_DownSide + (offset * m_ItemSize.y + scrollOffset) * Vector3.down * 0.3f;
        }
        ...
        protected virtual Vector3 GetObjectSize(GameObject g)
        {
            ...
            if (rend)
            {
                itemSize.x = Vector3.Scale(g.transform.lossyScale, rend.bounds.extents).x + padding;
                itemSize.y = Vector3.Scale(g.transform.lossyScale, rend.bounds.extents).y + padding;
                itemSize.z = Vector3.Scale(g.transform.lossyScale, rend.bounds.extents).z + padding;
            }
            return itemSize;
        }
